...
if (TRY_TO_MAKE_FONTS_MORE_CRISP) {
    ...
    context.fillText = function(text,x,y,maxWidth) {
        ...
        if (!window.fontcanvas) {
            ...
            fontcanvas.width = canvas.width * 4;
            fontcanvas.height = canvas.height * 4;
        ...
        let bigsize = smallsize * 4;
        fontctx.font = bigsize + "px PressStart2P";;
        fontctx.fillStyle = context.fillStyle;
        fontctx.fillText(text,x*4,y*4);//,maxWidth);

        // now copy it while scaling the text bitmap down
        context.drawImage(fontcanvas,0,0,fontcanvas.width/4,fontcanvas.height/4);
